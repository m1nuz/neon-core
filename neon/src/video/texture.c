...
extern TEXTURE
texgen_radial_gradient(int width, int height, COLOR c0, COLOR c1, int radius) {
    ...
    // upload to OpenGL
    TEXTURE tex = new_texture2D(&(IMAGE_DATA){.internalformat = video.i_rgb8,
                                              .width = width,
                                              .height = height,
                                              .format = video.f_rgb,
...
extern TEXTURE
texgen_color(int width, int height, RGB_COLOR color) {
    RGB_COLOR *pixels = malloc(width * height * sizeof(RGB_COLOR));
